...
def process_df(df, output): 
    problems = df.apply(__build_problem_raw, axis=1)
    solutions = df.apply(__build_solution_raw, axis=1)
    
    instructions = pd.Series([ build_prompt(*x) for x in zip(df.index, problems, solutions) ])
